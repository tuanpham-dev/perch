...
A VSCode-style web UI for your terminals. Open folders as projects — each backed by its own terminal session — browse them in a sidebar, open them as tabbed terminals, and manage sessions/windows — all from the browser over WebSockets. Terminals live in a bundled daemon, so they keep running when the server restarts, redeploys or crashes, and come back (history, working directories, names) after a reboot. Rendered rendered by [xterm.js](https://xtermjs.org/) — or the optional [ghostty-web](https://www.npmjs.com/package/ghostty-web) (Ghostty's WASM terminal core) engine, installable from the extensions registry — selectable in Settings.
...
- **Rebindable keybindings** — every shortcut can be remapped from the Keyboard Shortcuts editor (gear menu → Keyboard Shortcuts); see [Keyboard & mouse](#keyboard--mouse) below for the defaults.
...
Config (`PORT`, `AUTH_TOKEN`, `ALLOWED_HOSTS`, `NEW_SESSION_CWD`, `APP_NAME`, `PROXY_DOMAIN`, `EXTENSION_REGISTRY`) goes in `~/.local/share/perch/server/.env` — see [Production](#production) below for what each does. Without a service manager, `start`/`stop`/`restart` fall back to running the server in the background directly.
...
The extension's id is `publisher.name` (falling back to its folder name). `contributes.themes`/`contributes.iconThemes` point at real, unmodified VS Code color-theme / icon-theme JSON — comments and trailing commas are tolerated, and one level of `include` is resolved. `perch.client`/`perch.server` are both optional; either, neither, or both may be set.
...
Not a VS Code manifest concept (VS Code's own font contributions are icon-theme glyph fonts only) — `contributes.fonts` is Perch's own extension of the manifest, for terminal text fonts. Each entry is a named **group** bundling one or more font families:
...
`perch.client` is a plain ESM module (no JSX, no bundler) exporting `activate(ctx)`:
...
- `ctx.registerFileOpenInterceptor(intercept)` — `intercept(path)` resolves `true` to claim a file open that would otherwise reach the editor (after `"default"`-mode viewer matching); a throw or `false` falls through, so a broken interceptor never blocks opening (how file-guard catches binary and oversized files)
- `ctx.registerSidebarPanel({ id, title, icon?, location?, defaultTab?, defaultCollapsed?, order?, focusBinding?, component })` — a sidebar surface. `location: "tab"` (the default) adds its own tab to the sidebar's tab strip, alongside the built-in Explorer tab; `location: "explorer"` renders it as an accordion section inside the Explorer tab beside PROJECTS/FILES, and `"run"`/`"commands"` do the same inside the Run and Commands tabs (how tasks, snippets and command-history add their sections), taking part in the accordion's drag-reorder/collapse/resize persistence under its namespaced id, starting collapsed when `defaultCollapsed` is set, and always registering a "Sidebar: Focus `<title>`" command. `location` sets only the panel's *default* home — the user can move any panel into any tab from its header's context menu, and both the panel's focus command and `ctx.app.revealSidebarPanel` follow it wherever it ends up. `icon` is a codicon name (default `"extensions"`); `focusBinding` (a keybindings.ts-style combo, e.g. `"ctrl+shift+KeyG"`) gives that focus command a default binding — for `"tab"` panels, omitting it omits the command entirely. The component receives optional host props: `actionsTarget` (a `HTMLDivElement | null` to portal header-row buttons into — same portal mechanism `registerFileViewer`'s `toolbarTarget` uses), `showMenu(x, y, items)` (the shared context menu), and `confirmDialog(message, confirmLabel?)` (the app's confirm dialog, resolving `true` on confirm)
- `ctx.registerSidebarTab({ id, title, icon, side? })` — an empty sidebar tab the user fills by moving panes into it; returns a handle with `update`, `reveal` and `remove`
- `ctx.registerSidebarTabMenuItem({ id, label, order?, isVisible, onClick })` — an item in the sidebar tab strip's right-click menu, evaluated against `{ tabId, side }`
- `ctx.registerWindowAction({ id, icon, title, isVisible, onClick, showInTabBar? })` — an icon button on PROJECTS-tree terminal rows (next to the built-in kill button), shown only where `isVisible({ sessionName, windowIndex, cwd, command })` returns true — `command` is the window's current foreground command, so an action can target e.g. only windows running `claude`; `showInTabBar: true` also renders it in the tab bar next to whichever terminal window tab is focused
- `ctx.registerFileMenuItem({ id, label, icon?, order?, isVisible, onClick })` — an item in the FILES tree's right-click menu for a single file or folder, after the built-in items: `isVisible(path, isDir)`, `onClick(path)`
- `ctx.registerTabGroupMenuItem({ id, label, icon?, order?, isVisible, onClick })` — an item in a tab-group chip's windows dropdown, below New Window, evaluated against `{ sessionName, cwd }`
...
- `ctx.registerSessionDecorationProvider({ id, provideWindowDecoration, onClick? })` — badges on PROJECTS terminal rows: `provideWindowDecoration({ sessionName, windowIndex, cwd, command })` returns `{ badge, tooltip?, className? }`; `onClick(anchorRect, ctx)` lets the extension open its own popover (how subagent-viewer shows agent counts and its details panel). Same sync-from-cache + `refresh()` contract
- `ctx.registerEditor({ id, label, capabilities, openFile, openDiff?, openMerge? })` — supplies an editor for the Settings → Editor picker (declared in the manifest as `contributes.editors`, so the picker lists it without activating anything). `capabilities` is any of `"file"`, `"diff"`, `"merge"`; whatever isn't declared falls back to core's nvim editor. Callers reach it through `ctx.app.openInEditor/openDiff/openMerge` — see [docs/EXTENSION_API.md](docs/EXTENSION_API.md#editors--registereditor)
- `ctx.registerTerminalEngine({ id, label, create })` — supplies a terminal engine (`create` implements the `CreateTerminalEngine` seam — see `extensions/_shared/terminalEngineTypes.ts`); declare it in the manifest as `contributes.terminalEngines` too, so the Settings → Terminal engine select lists it without activating anything, and only the selected engine's extension is loaded; the engine setting stores its namespaced id (how the bundled xterm-engine and the registry's ghostty-engine ship)
...
- `ctx.registerAppOverlay({ id, component })` — renders a component once over the editor area, on top of whatever tab is active (unlike a terminal accessory, which mounts only inside a focused terminal); the host draws it into a `pointer-events: none` layer, so the component opts its own interactive parts back in
- `ctx.registerStatusBarItem({ id, title?, placement?, order?, visibilitySetting?, component })` — a compact readout in the bottom status bar. `placement` is `"left"` or `"right"` (default `"right"`, where the app's own terminal count also sits) and `order` sorts within that group. `title` names the item in the gear menu's Status Bar list, where the user switches it on and off (default: the extension's display name). `visibilitySetting` is the dotted key of a boolean setting from your own manifest; give one and that setting becomes the item's switch — the list reads and writes it instead of the app's own hidden list, and the item renders unless the value is `false` — so an extension that already offers "show this in the status bar" keeps one control rather than gaining a second (the bundled Git extension does this with `gitScm.statusBar`). The component receives a `context` prop with `mobilePointer`, `showMenu(x, y, items)`, and `openPopover(anchorRect, content)` / `closePopover()` — a floating panel above the bar that the host positions and dismisses, and that `openPopover` toggles when the same item calls it again (how the bundled ports extension shows its port list and forward command, and how the external Claude usage extension shows its token counter). On phones the bar stays but goes compact, so an item renders there too: use `mobilePointer` to drop it to its icon and number, the way the app's own readouts do.
...
- `ctx.app.closeViewerTab(viewerId, path)` — closes one of this extension's own viewer tabs, with no unsaved-changes confirm (for a draft tab your own save flow already handled)
...
- `ctx.app.setSidebarBadge(panelId, badge)` — sets (a number) or clears (`null`/`0`) the count badge on one of this extension's own sidebar panels
- `ctx.app.setSidebarPanelVisible(panelId, visible)` — hides or shows one of this extension's own panels when it has nothing to show in the current context; a tab left with no visible sections drops out of the strip
- `ctx.app.revealSidebarPanel(panelId)` — reveals one of this extension's own sidebar panels (reveals the sidebar, switches to its tab, expands and focuses its section) — what a command that opens the panel's UI should call, since unlike the panel's own focus command it never toggles an already-active panel shut
- `ctx.app.openSessionWindow(sessionName, opts?: { createCwd? })` — opens a session's current window as a window-tab; with `createCwd`, a session that doesn't exist yet is created there first (how a fresh checkout becomes a session in one step)
- `ctx.app.killSession(sessionName)` — kills a session and closes its tabs. Runs **no** confirmation of its own — the caller owns the prompt, so confirm before calling
- `ctx.app.newWorktree(opts?: { runCommandIndex? })` / `ctx.app.cleanUpWorktrees()` — reveal PROJECTS and start its create-worktree form or its Clean Up Worktrees flow
- `ctx.app.confirmDialog(message, confirmLabel?)` / `ctx.app.promptDialog(message, defaultValue?)` / `ctx.app.pickItem({ title, items, current?, placeholder? })` / `ctx.app.pickIcon(opts?: { title?, current? })` — the app's own confirm, text-prompt, filterable-list and codicon-picker dialogs; each resolves `true`, the entered text, or the chosen id or icon name (`false`/`null` on cancel)
- `ctx.app.getFileIcon(fileName)` / `ctx.app.getFolderIcon(folderName, expanded)` / `ctx.app.onDidChangeIconTheme(cb)` — read-only lookups against the active icon theme, the same icons the FILES tree shows
- `ctx.app.getThemeColors()` / `ctx.app.getTokenColors()` / `ctx.app.onDidChangeColorTheme(cb)` — the active color theme's raw workbench colors and TextMate token rules, for an extension running its own syntax highlighter
- `ctx.app.openInEditor(path, line?)` — opens a path in the editor the `editor` setting selects (nvim by default), skipping viewer matching
- `ctx.app.openDiff(req)` / `ctx.app.openMerge(req)` — a two-sided diff, or a conflicted file, in the selected editor; resolves `false` when no editor claims that capability, so fall back to your own view
- `ctx.app.canPreview(path)` / `ctx.app.openPreview(path)` — whether any registered viewer can show a rendered preview of a path (Markdown, JSON/YAML, CSV…), and opening it, even when that viewer belongs to another extension
- `ctx.app.agentForWindow(session, windowIndex)` / `ctx.app.agentForSession(session)` — which agent (if any) runs in a terminal, with its process id; resolves `null` / `[]` for a missing session and never throws
- `ctx.app.executeCommand(commandId)` / `ctx.app.getCommands()` — run a globally dispatchable built-in or extension command by id, and list the ones that can be run
- `ctx.app.focusActiveTerminal()` — hands keyboard focus back to the focused terminal after an extension dialog or overlay closes
- `ctx.app.consumeFindInFolderGlob()` — one-shot handoff of the glob from the FILES tree's "Find in Folder…" item; only the `search` extension is expected to call it
...
`perch.server` is a plain ESM module exporting `activate({ router, log, getSettings, ai, secrets, host })` and optionally `deactivate()` — `router` is an Express router mounted at `/api/ext/<extensionId>` for as long as the extension stays enabled; disabling/uninstalling unmounts the routes immediately and calls `deactivate()` so timers, watchers and sockets can be torn down, though the loaded module itself stays resident until the server restarts (Node can't unload an ES module). `getSettings()` returns this extension's current settings — see [Settings](#settings) above. `host` is the curated core-services API (ports, sessions, worktrees, agents, agent hooks, AI, secrets, terminal engines, events; see docs/EXTENSION_API.md), e.g. `host.ports.list()`/`host.ports.find(port)` (listening ports attributed to terminals, the same data the WS tunnel's security gate uses) and `host.events.onApiMutation(cb)` (fires after any mutating core API call — invalidate caches that mirror on-disk state; returns an unsubscribe, and all of an extension's subscriptions drop when its hook unmounts).

Enabling, installing, or disabling an extension applies live: a newly enabled client entry is activated, and a disabled one has its optional `deactivate()` export called and its contributions unregistered. Terminal engines activate only when a terminal actually uses them. Themes and icon themes need no activation at all.

### Security

Installing an extension with a `perch.server` entry runs its code as the server process's user, and a `perch.client` entry runs with full access to the page (same origin, same DOM) — identical in kind to the access this app already gives you through the terminal itself, but only install extensions you trust.
...
extensions/ Bundled extensions (previews, git, search, ports, tasks, snippets, touch keys, agents, default theme/icons/font) — see Bundled extensions
cli/        tunnel.mjs — standalone port-forwarding client, served at GET /tunnel.mjs
bin/        perch — entry point of the CLI for managing an installed instance, see Install
