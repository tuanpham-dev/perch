...
Perch's guiding structural rule:
...
"Core" is the infrastructure a browser terminal workspace cannot exist without; every
...
| --- | --- |
| Terminal attach (server `terminals.ts`, `multiplexer.ts`, `wsAttach.ts`; client `TerminalView`) | The product itself: session/window listing, WS attach, input pipeline (sticky Ctrl, local echo, mouse reports). |
...
| Engine-support helpers (`mouseReports.ts`, `terminalLinks.ts`, `contrast.ts`, `selectionText.ts`, `lib/terminalInput.ts`) | Shared by core (TerminalView, touchSelect, localEcho) *and* extension engines/accessories — exposed to extension bundles through the `@perch/engine-support` shim (below), never duplicated. |
...
- **ports** — the Ports status-bar item and its popover (list/kill via the server `host.ports` API), plus the port proxy viewer
- **subagent-viewer** — Claude Code subagent count badges on PROJECTS-tree window rows + the details popover (session-decoration provider; its server hook reads Claude Code's on-disk transcripts)
- **worktrees** — palette commands that open the PROJECTS tree's create-worktree form (`ctx.app.newWorktree`) and start Clean Up Worktrees (`ctx.app.cleanUpWorktrees`). Worktrees themselves are core: `server/src/gitWorktrees.ts` owns every `git worktree` call, `/api/git/worktrees` serves the tree's middle level, and `client/src/lib/projects.ts`'s `projectTree` groups sessions by repository — an extension can't contribute rows to a core tree
- **xterm-engine** — the xterm.js terminal engine and the default. **Required builtin** (`perch.required: true`): it is the app's rendering floor, so the platform refuses to disable or uninstall it, and ignores stale state-file entries for it. The engine registry falls back to it whenever the selected engine is missing. (The alternative `ghostty-engine`, the ghostty-web WASM engine, is no longer bundled — it lives in the `perch-extensions` registry as an optional install.)
- **agents** — the bundled agent definitions (Claude Code, OpenAI Codex) via `contributes.agents`
- **command-history** — the HISTORY section of the Commands tab + the `!` quick-switcher mode
- **file-guard** — a file-open interceptor for binary/oversized files
- **snippets** — the SNIPPETS section of the Commands tab + quick-switcher results
- **tasks** — the TASKS section of the Run tab
...
- **Assets** — `plastic-legacy-theme`, `seti-icons`, `ibm-plex-mono` (default look; hard fallbacks in core cover the gap when disabled). `mono-fonts` is not bundled — it lives in the `perch-extensions` registry
...
| `registerSidebarPanel` | A sidebar surface — `location: "tab"` (own tab, default) or an accordion section in the Explorer (`"explorer"`, beside PROJECTS/FILES), Run (`"run"`) or Commands (`"commands"`) tab (with persisted order/collapse/size under its namespaced id and an always-registered focus command). |
| `registerWindowAction` | Icon button on PROJECTS-tree window rows (+ optionally the tab bar). |
...
| `registerSessionDecorationProvider` | Badges on PROJECTS-tree window rows with an `onClick(anchorRect, ctx)` for extension-owned popovers. |
| `registerTerminalEngine` | A `CreateTerminalEngine` factory (the seam in `client/src/engines/types.ts`); resolved against the terminal-engine setting once the extension list is known, activating only the owning extension. |
...
| Manifest contributions | `contributes.themes` / `iconThemes` / `fonts` / `configuration` / `editors` / `terminalEngines` / `agents` — data-only, no code runs. |

Server (`activate({ router, log, getSettings, ai, secrets, host })`; the full `host` is in [EXTENSION_API.md's Server API table](EXTENSION_API.md#server-api)):
...
| `host.ports.list()/find(port)` | Core port attribution (listening ports owned by terminals) — shared with the tunnel gate. |
...
- **Host-instance shims** (`extensions/_shared/shims/`, aliased by `extensions/build.mjs`): `react`, `react-dom`, `react-dom/client`, `react/jsx-runtime`, and `@perch/engine-support` re-export the host's own instances via `window.__perchModules` (set in `client/src/main.tsx`). Anything with identity or shared state — React, the synthetic-select-start Symbol, `findCandidates`, `whenMatches` — must come from here, never a bundled copy.
- **Source copies** (`extensions/_shared/*.ts(x)`): small stateless helpers (e.g. Icon, `useListNavigation`, clipboard, the `terminalEngineTypes` seam copy) each extension inlines at build time; structural typing keeps them interoperable with the host's originals.

### The required-builtin mechanism

`perch.required: true` in a manifest — honored **only** for bundled
...
- Quick-switcher sources in bundled extensions: git-scm branch switching,
  ...
  adopted by command-history and snippets).
